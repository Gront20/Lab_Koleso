...
TProduceFunctionKoleso=class(TProduceFunction)
private
      Fr: real;
      Fl: real;
      ...
      function GetL: real;
      procedure SetL(AValue: real);
      function CalcFunctionIntegral(AAlfa: real): real;
      function CalcKolesoIntegral(count: integer): real;
public
      constructor Create(AR,AL,AMax,AMin,ADelta: real); overload;
      constructor Create; overload;
      destructor Destroy; override;
published
     property RFunction: real read GetR write SetR;
     property LFunction: real read GetL write SetL;
end;
...
procedure TProduceFunctionKoleso.SetR(AValue: real);
begin
   try
      if (AValue<0) then
      begin
          Fr:=DEFAULT_BETA;
          Raise Exception.Create('Неправильные значение радиуса колеса!');
      end
      else Fr:=AValue;
      OnCalcValues;
   except
...
//---------------------------------------------------------------------------
function TProduceFunctionKoleso.GetR: real;
begin
    result:=Fr;
end;
...
procedure TProduceFunctionKoleso.SetL(AValue: real);
begin
   try
      if (AValue<0) then
      begin
          Fl:=DEFAULT_BETA;
          Raise Exception.Create('Неправильные значение положения точки на колесе!');
      end
      else Fl:=AValue;
      OnCalcValues;
   except
...
//---------------------------------------------------------------------------
function TProduceFunctionKoleso.GetL: real;
begin
    result:=Fl;
end;
//---------------------------------------------------------------------------
function TProduceFunctionKoleso.CalcFunctionIntegral(AAlfa: real): real;
begin
            Result:=sqrt(Fl*Fl+Fr*Fr-2*Fr*Fl*cos(AAlfa));
end;

//---------------------------------------------------------------------------
function TProduceFunctionKoleso.CalcKolesoIntegral(count: integer): real;
var
  i: integer;
  PValue1,PValue2: ^real;
begin
   Result:=0;
   if count>1 then
   begin
             PValue1:=Items[0];
             PValue2:=Items[count-1];
             Result:=(CalcFunctionIntegral(PValue1^)+CalcFunctionIntegral(PValue2^))/2;
             if count>2 then
             begin
               for i:= 1 to count-2 do
                 begin
                   PValue1:=Items[i];
                   Result:=Result+CalcFunctionIntegral(PValue1^);
                 end;
             end;
             Result:=Result*Delta;
   end;
...
procedure TProduceFunctionKoleso.OnCalcValues;
var
  i: integer;
  PValue: ^real;
...
constructor TProduceFunctionKoleso.Create(AR,AL,AMax,AMin,ADelta: real);
begin
  try
     FFunctionValues:=TList.Create;
     Fr:=AR;
...
//---------------------------------------------------------------------------
constructor TProduceFunctionKoleso.Create;
begin
  try
     FFunctionValues:=TList.Create;
     Fr:=DEFAULT_BETA;
...
//---------------------------------------------------------------------------
destructor TProduceFunctionKoleso.Destroy;
var
   i: Integer;
begin
   if Count<>0 then
...
begin
   ...
   temp:=Height-temp;
   FCanvas.TextOut(FOffset.x+x-10,FOffset.y+trunc(temp)-10,'S');
...
begin
  ...
  if not FPrecisionDiv then
   ...
   FCanvas.TextOut(FOffset.x+trunc(temp)-10,FOffset.y+y-20,'A');
 ...
 if Edit1.Modified then
    begin
       try
         ...
         ShowMessage('Неправильное значение радиуса колеса !');
         Edit1.SetFocus;
       end;
    end;
end;
 ...
 if Edit3.Modified then
    begin
       try
         ...
         ShowMessage('Неправильное значение расстояния до точки !');
         Edit3.SetFocus;
       end;
    end;
end;
...
begin
   pnt.X:=40;
   pnt.Y:=40;
   pfr:=TProduceFunctionKoleso.Create(1,1,(2*VPI),0,0.001);
   Edit1.Text:=FloatToStr(pfr.RFunction);
   Edit3.Text:=FloatToStr(pfr.LFunction);
  ...
  begin
         ...
         begin
            ...
            begin
                      ...
                      begin
                         Image1.Hint := 'Alfa='+FloatToStr(SimpleRoundTo(p1.x, -1*l))+';Distance='+FloatToStr(p1.y);
